...
pub unsafe fn set_click_through(hwnd: HWND, enabled: bool) {
    ...
    if enabled { style |= WS_EX_TRANSPARENT.0; } else { style &= !WS_EX_TRANSPARENT.0; }
    SetWindowLongW(hwnd, GWL_EXSTYLE, style as i32);
}

pub unsafe fn is_minimized(hwnd: HWND) -> bool { IsIconic(hwnd).as_bool() }
pub unsafe fn minimize_window(hwnd: HWND) { let _ = ShowWindow(hwnd, SW_MINIMIZE); }
